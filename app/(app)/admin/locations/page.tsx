...
export default async function LocationsPage({
  ...
  const [rows, total] = await Promise.all([
    prisma.location.findMany({ where, orderBy: { locationName: "asc" }, skip, take }),
    ...
    { key: "focus", header: "Commodity focus", cell: (r) => r.commodityFocus ?? "—" },
    {
      ...
      cell: (r) => (
        <div className="flex justify-end gap-1">
          <EntityFormDialog title="Edit location" fields={fields} action={updateLocation} values={{ id: r.id, locationName: r.locationName, locationType: r.locationType ?? "", region: r.region ?? "", commodityFocus: r.commodityFocus ?? "", keyPersonnel: r.keyPersonnel ?? "", notes: r.notes ?? "" }} submitLabel="Save changes" trigger={<Button variant="ghost" size="icon-sm" aria-label="Edit"><Pencil /></Button>} />
          <ConfirmButton title="Delete location" description={`Delete ${r.locationName}?`} confirmLabel="Delete" action={deleteLocation.bind(null, r.id)} trigger={<Button variant="ghost" size="icon-sm" aria-label="Delete"><Trash2 /></Button>} />
        </div>
      ),
    },
  ]
    ...
    <>
      ...
      <div className="space-y-4">
        <DataTableToolbar
          ...
          filters={[{ key: "type", label: "Type", options: LOCATION_TYPES.map((t) => ({ label: t, value: t })) }]}
        >
