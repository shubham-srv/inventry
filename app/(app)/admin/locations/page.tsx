...
import { selectableLocationTypes, allLocationTypes, locationTypeLabel } from "@/lib/location-types"
...
export default async function LocationsPage({
  ...
  const [rows, total, regions, countries, typeOptionsRaw, allTypes] = await Promise.all([
    prisma.location.findMany({ where, include: { region: true, country: true, locationType: true }, orderBy: { locationName: "asc" }, skip, take }),
    ...
    // Only active types are offered for NEW sites; the filter below lists every
    // type, so a site on a retired one can still be found.
  ...
  const fields: Field[] = [
    ...
    { name: "locationTypeId", label: "Type", type: "select", required: true, placeholder: "Select type", options: typeOptionsRaw.map((t) => ({ label: locationTypeLabel(t.name, t.appliesTo), value: String(t.id) })), description: "Determines whether this site can be mapped to growers, vendors, or either. A location with no type is pickable by neither. Manage the list at Location types." },
  ...
  const columns: Column<Row>[] = [
    ...
    { key: "type", header: "Type", cell: (r) => r.locationType?.name ?? "—" },
    ...
    <>
      ...
      <div className="space-y-4">
        <DataTableToolbar
          ...
          filters={[
            { key: "type", label: "Type", options: allTypes.map((t) => ({ label: t.name, value: String(t.id) })) },
